# Forge — Parallel Candidate Execution — Design Spec

**Status:** Approved (2026-06-28). A beyond-design extension to the v4 Population strategy ([Population spec](2026-06-28-forge-population-search-design.md), which shipped candidates running *sequentially* and named parallel execution as a refinement).
...
- **Candidates are already fully isolated** — candidate 0 in `goal_dir/.forge`, candidates 1…N−1 each in their own `goal_dir/.forge/candidates/{i}/.forge` with a private copied suite. No shared mutable state, so they can run concurrently without interfering.
...
- **Determinism is preserved** — results are collected **in candidate-index order** (not completion order), and `select_best` ranks deterministically (ties → lowest index). So parallelism changes *only the speed*, never *which candidate wins* or *what lands at `goal_dir/.forge/best`*. Existing Population tests produce identical outcomes.
