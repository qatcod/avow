# Forge — The Supervisor (event-triggered trajectory guardian) — Design Spec

**Status:** Approved (2026-06-28). The last agent of the original [4-agent design](2026-06-26-forge-design.md) (§"Agents"): "Supervisor — judge trajectory → redirect / change strategy / escalate; never enforces; runs only when a trouble-signal fires."
...
`forge supervise <run_jsonl> <goal_file> [--config]` CLI — reads a recorded `run.jsonl`, reconstructs the trajectory, and prints the Supervisor's verdict for any past run.
...
- CLI: `forge supervise` offline (monkeypatched client) reading a small `run.jsonl` → prints the verdict.
