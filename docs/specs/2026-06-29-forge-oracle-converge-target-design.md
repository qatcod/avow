# Forge — Reference-Oracle as a Converge Target — Design Spec

**Status:** Approved (2026-06-29). A beyond-design upgrade to the reference-oracle ([oracle spec](2026-06-27-forge-reference-oracle-design.md), which shipped the oracle as a *post-green signal* and named "the reference as a converge target" as explicit future work).
...
- `forge/loop.py`: the `write_tests` block gains the oracle-converge-target hook (imports `generate_oracle` from `forge.oracle` and `TestFile` from `forge.examiner`); `solve` already takes `oracle_client`. No new `solve` parameter.
