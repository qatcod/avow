# Forge — Autonomous Build-and-Improve Loop — Design Spec
...
So Forge's job is to **synthesize a trustworthy verifier** for software goals, where no physics simulator exists.
...
**Out (documented roadmap, NOT built):** real-world business outcomes (e.g. "$10K MRR Shopify store"). Those have no sandbox verifier; they require a human/market round and are a later strategy, not core scope. Forge can autonomously *build* such a system; it cannot autonomously *verify* the business outcome.

## The honest position on verification

You cannot reach **certainty** without ground truth (a human or a simulator). With two AI agents alone, "all tests pass" means "two AIs agreed," not "the goal was met." Forge does **not** pretend otherwise. Instead it drives the probability that *agreement ≈ correctness* as high as possible and **reports a calibrated confidence**, escalating to a human only at measured uncertainty. The verifier is treated as the product's hard part and its moat.
...
- **Cost.** Multi-agent loops burn tokens; the budget cap is the primary economic control. For many tasks a skilled dev is cheaper by hand — Forge earns its keep on tasks that are *both* verifiable *and* tedious/large.
