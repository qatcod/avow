# Forge v3 — The Expand Phase — Implementation Plan
...
**Architecture:** A new `forge/ideator.py` (LLM proposes ranked ideas + a pure `select_idea` leash) and `forge/improve.py` (an orchestrator that wraps the existing `solve()` — converge → ideate → leash → grow the frozen suite → re-converge). A `forge improve` CLI. `solve()` itself is unchanged.

**Tech Stack:** Python 3.12 · `anthropic` structured outputs · reuses `forge.loop.solve` / `forge.examiner` / `forge.config`.
...
**Files:**
- Create: `/Users/qatadaha/Coding/forge/forge/ideator.py`
- Test: `/Users/qatadaha/Coding/forge/tests/test_ideator.py`
...
Run: `cd /Users/qatadaha/Coding/forge && source .venv/bin/activate && python -m pytest tests/test_ideator.py -q`
Expected: FAIL — `ModuleNotFoundError: No module named 'forge.ideator'`.

- [ ] **Step 3: Write `forge/ideator.py`**
...
```bash
cd /Users/qatadaha/Coding/forge && git add forge/ideator.py tests/test_ideator.py && git commit -m "feat: Ideator — propose ranked next-feature ideas with verifier + risk"
```
...
**Files:**
- Modify: `/Users/qatadaha/Coding/forge/forge/ideator.py`
- Test: `/Users/qatadaha/Coding/forge/tests/test_select_idea.py`
...
Run: `cd /Users/qatadaha/Coding/forge && source .venv/bin/activate && python -m pytest tests/test_select_idea.py -q`
Expected: FAIL — `ImportError: cannot import name 'select_idea'`.

- [ ] **Step 3: Append to `forge/ideator.py`**
...
```bash
cd /Users/qatadaha/Coding/forge && git add forge/ideator.py tests/test_select_idea.py && git commit -m "feat: select_idea leash — auto-pursue objective low-risk, escalate the rest"
```
...
```bash
cd /Users/qatadaha/Coding/forge && git add forge/config.py tests/test_config.py && git commit -m "feat: expand-phase settings on RunConfig"
```
...
**Files:**
- Create: `/Users/qatadaha/Coding/forge/forge/improve.py`
- Test: `/Users/qatadaha/Coding/forge/tests/test_improve.py`
...
Run: `cd /Users/qatadaha/Coding/forge && source .venv/bin/activate && python -m pytest tests/test_improve.py -q`
Expected: FAIL — `ModuleNotFoundError: No module named 'forge.improve'`.

- [ ] **Step 3: Write `forge/improve.py`**
...
```bash
cd /Users/qatadaha/Coding/forge && git add forge/improve.py tests/test_improve.py && git commit -m "feat: improve() orchestrator — converge, ideate, grow the suite, re-converge"
```

---

### Task 5: `forge improve` CLI

**Files:**
- Modify: `/Users/qatadaha/Coding/forge/forge/cli.py`
- Test: `/Users/qatadaha/Coding/forge/tests/test_cli_improve.py`

**Interfaces:**
- New subcommand: `forge improve <goal_dir> [--config forge.yaml] [--no-llm-verify]`. Builds the Examiner + Builder + a shared verify client (unless `--no-llm-verify`), runs `improve(...)` passing the client as `ideator_client`/`intent_client`/`property_client`, prints the verdict + per-round lines. The existing `solve`/`mutate`/`intent-check`/`verify`/`propertize` subcommands are unchanged.
...
Run: `cd /Users/qatadaha/Coding/forge && source .venv/bin/activate && python -m pytest tests/test_cli_improve.py -q`
...
Run: `cd /Users/qatadaha/Coding/forge && source .venv/bin/activate && python -m pytest tests/test_cli_improve.py tests/test_cli.py -q`
...
Expected: all tests PASS, 0 warnings; `forge --help` lists `solve`, `mutate`, `intent-check`, `verify`, `propertize`, `improve`.

- [ ] **Step 6: Prepare commit** (run only when greenlit)

```bash
cd /Users/qatadaha/Coding/forge && git add forge/cli.py tests/test_cli_improve.py && git commit -m "feat: forge improve CLI — the self-improvement expand loop"
```
...
`forge improve ~/Coding/forge-demo-full` → converges on the goal, then the Ideator proposes the next feature; objective low-risk ones auto-pursue (the suite grows and re-converges), riskier ones would escalate. Prints a line per round.
