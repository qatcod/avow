# Avow v2.3 — Calibrated Confidence Aggregation — Implementation Plan
...
**Architecture:** A new pure-math `avow/confidence.py` aggregator (no LLM), wired into the loop's green-converge branch (replacing the binary hold-out gate with a hold-out *score* that feeds confidence), plus a standalone `avow verify` CLI.

**Tech Stack:** Python 3.12 · stdlib only for the aggregator · integrates with `avow.config`/`avow.loop`/`avow.runner`/`avow.mutation`/`avow.backtranslation`.
...
**Files:**
- Create: `/Users/qatadaha/Coding/avow/avow/confidence.py`
- Test: `/Users/qatadaha/Coding/avow/tests/test_confidence.py`
...
Run: `cd /Users/qatadaha/Coding/avow && source .venv/bin/activate && python -m pytest tests/test_confidence.py -q`
Expected: FAIL — `ModuleNotFoundError: No module named 'avow.confidence'`.

- [ ] **Step 3: Write `avow/confidence.py`**
...
```bash
cd /Users/qatadaha/Coding/avow && git add avow/confidence.py tests/test_confidence.py && git commit -m "feat: pure weighted-average confidence aggregator with renormalization"
```
...
```bash
cd /Users/qatadaha/Coding/avow && git add avow/config.py tests/test_config.py && git commit -m "feat: confidence threshold/weights/gating on RunConfig"
```
...
Run: `cd /Users/qatadaha/Coding/avow && source .venv/bin/activate && python -m pytest tests/test_loop.py::test_loop_low_confidence_flags -q`
...
```bash
cd /Users/qatadaha/Coding/avow && git add avow/loop.py tests/test_loop.py && git commit -m "feat: rewire solve() done around calibrated confidence with escalation"
```

---

### Task 4: `avow verify` CLI — confidence for any artifact

**Files:**
- Modify: `/Users/qatadaha/Coding/avow/avow/cli.py`
- Test: `/Users/qatadaha/Coding/avow/tests/test_cli_verify.py`

**Interfaces:**
- New subcommand: `avow verify <solution_dir> <tests_dir> <goal_file> [--config avow.yaml] [--llm]`. Runs `run_mutation_testing` (AST-only unless `--llm`) + `run_intent_check`, aggregates `{mutation, intent}` (no hold-out in the standalone path), and prints the confidence + breakdown + the per-verifier details. Omits the mutation signal (with a warning) if the suite isn't green on the unmutated solution (`mr.baseline_green is False`). The existing `solve`/`mutate`/`intent-check` subcommands are unchanged.
...
Run: `cd /Users/qatadaha/Coding/avow && source .venv/bin/activate && python -m pytest tests/test_cli_verify.py -q`
...
Run: `cd /Users/qatadaha/Coding/avow && source .venv/bin/activate && python -m pytest tests/test_cli_verify.py tests/test_cli.py tests/test_cli_mutate.py tests/test_cli_intent.py -q`
...
Expected: all tests PASS, 0 warnings; `avow --help` lists `solve`, `mutate`, `intent-check`, `verify`.

- [ ] **Step 6: Prepare commit** (run only when greenlit)

```bash
cd /Users/qatadaha/Coding/avow && git add avow/cli.py tests/test_cli_verify.py && git commit -m "feat: avow verify CLI — aggregate all verifiers into a confidence for any artifact"
```
...
`avow verify ~/Coding/avow-demo/sol ~/Coding/avow-demo/tests_frozen ~/Coding/avow-demo/goal.md` → prints a single confidence number with the per-signal breakdown (mutation kill-rate + intent match), the survivor count, the inferred goal, and any divergences — "how much should I trust this artifact?" in one command.
