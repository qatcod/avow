# Forge — Reference-Oracle Differential Testing — Implementation Plan
...
**Architecture:** A new `forge/oracle.py` (LLM emits a reference + a Hypothesis diff test; `run_oracle_check` runs it in an ephemeral dir → agreement), wired into the loop's post-green block (a new `oracle` confidence signal + an `oracle_floor`). A `forge oracle` CLI. Mirrors the mutation signal's shape.

**Tech Stack:** Python 3.12 · `anthropic` structured outputs · `hypothesis` · `pytest-json-report` · reuses `forge.scoring`/`forge.loop`/`forge.config`/`forge.confidence`.
...
- Reuses verified interfaces (do NOT modify): `parse_report(data) -> TestResult(passed, failed, errors, total, failures: list[FailureInfo(nodeid, message)])` from `forge.scoring`; `aggregate_confidence(signals, weights)` already filters present (non-None, weight>0) signals and renormalizes; `RunConfig`; `Budget.charge_tokens`; `RunLog.record(AttemptRecord(...))`; `solve(...)`/`SolveResult` in `forge.loop`.
...
**Files:**
- Create: `/Users/qatadaha/Coding/forge/forge/oracle.py`
- Test: `/Users/qatadaha/Coding/forge/tests/test_oracle_gen.py`
...
Run: `cd /Users/qatadaha/Coding/forge && source .venv/bin/activate && python -m pytest tests/test_oracle_gen.py -q`
Expected: FAIL — `ModuleNotFoundError: No module named 'forge.oracle'`.

- [ ] **Step 3: Write `forge/oracle.py`**
...
```bash
cd /Users/qatadaha/Coding/forge && git add forge/oracle.py tests/test_oracle_gen.py && git commit -m "feat: generate a reference impl + Hypothesis differential test for a goal"
```
...
**Files:**
- Modify: `/Users/qatadaha/Coding/forge/forge/oracle.py`
- Test: `/Users/qatadaha/Coding/forge/tests/test_oracle_run.py`
...
Run: `cd /Users/qatadaha/Coding/forge && source .venv/bin/activate && python -m pytest tests/test_oracle_run.py -q`
Expected: FAIL — `ImportError: cannot import name 'run_oracle_check'`.

- [ ] **Step 3: Append to `forge/oracle.py`**
...
```bash
cd /Users/qatadaha/Coding/forge && git add forge/oracle.py tests/test_oracle_run.py && git commit -m "feat: run_oracle_check — differential-test a solution against an independent reference"
```
...
```bash
cd /Users/qatadaha/Coding/forge && git add forge/config.py tests/test_config.py && git commit -m "feat: reference-oracle settings + oracle confidence weight on RunConfig"
```
...
**Interfaces:**
- `solve` gains a keyword-only `oracle_client=None` (after `property_client`). Imports `run_oracle_check` from `forge.oracle`. `SolveResult` gains `oracle_agreement: float | None = None` (after `confidence_breakdown`).
...
Run: `cd /Users/qatadaha/Coding/forge && source .venv/bin/activate && python -m pytest tests/test_loop.py::test_loop_oracle_disagreement_floor -q`
...
```bash
cd /Users/qatadaha/Coding/forge && git add forge/loop.py tests/test_loop.py && git commit -m "feat: reference-oracle as a post-green confidence signal + agreement floor"
```

---

### Task 5: `forge oracle` CLI

**Files:**
- Modify: `/Users/qatadaha/Coding/forge/forge/cli.py`
- Test: `/Users/qatadaha/Coding/forge/tests/test_cli_oracle.py`

**Interfaces:**
- New subcommand: `forge oracle <solution_dir> <goal_file> [--config forge.yaml]`. Builds `anthropic.Anthropic()`, runs `run_oracle_check`, prints `oracle agreement: <x>` and any counterexample. The existing subcommands are unchanged.
...
Run: `cd /Users/qatadaha/Coding/forge && source .venv/bin/activate && python -m pytest tests/test_cli_oracle.py -q`
Expected: FAIL — argparse `invalid choice: 'oracle'`.

- [ ] **Step 3: Edit `forge/cli.py`**
...
Run: `cd /Users/qatadaha/Coding/forge && source .venv/bin/activate && python -m pytest tests/test_cli_oracle.py tests/test_cli.py -q`
Expected: PASS (oracle CLI + the unchanged subcommands).

- [ ] **Step 5: Run the whole suite + smoke the entry point**

Run: `cd /Users/qatadaha/Coding/forge && source .venv/bin/activate && python -m pytest -q && forge --help`
Expected: all tests PASS, 0 warnings; `forge --help` lists `solve`, `improve`, `mutate`, `intent-check`, `verify`, `propertize`, `oracle`.

- [ ] **Step 6: Prepare commit** (run only when greenlit)

```bash
cd /Users/qatadaha/Coding/forge && git add forge/cli.py tests/test_cli_oracle.py && git commit -m "feat: forge oracle CLI — differential-test any solution against a generated reference"
```
...
`forge oracle ~/Coding/forge-demo-full/.forge/best ~/Coding/forge-demo-full/goal.md` → generates an independent reference slugify and fuzzes the built solution against it; prints `oracle agreement: 1.0` (or a counterexample if they diverge).
