# Forge — Reference-Oracle as a Converge Target — Implementation Plan
...
**Tech Stack:** Python 3.12 · reuses `forge.oracle.generate_oracle` · `forge.examiner.TestFile` · `hypothesis` · `forge.loop`/`forge.config`.
...
- Reuses verified interfaces (do NOT modify): `generate_oracle(goal, client, model) -> tuple[_OraclePair | None, int, int]` from `forge.oracle`; `TestFile(path, content)` from `forge.examiner`; `_write_tests`/`split_suite`; `solve(...)` already has `oracle_client`.
...
```bash
cd /Users/qatadaha/Coding/forge && git add forge/config.py tests/test_config.py && git commit -m "feat: oracle_converge_target setting on RunConfig (off by default)"
```
...
**Interfaces:**
- No signature change (`solve` already takes `oracle_client`). Imports `generate_oracle` from `forge.oracle` and `TestFile` from `forge.examiner`.

**Edits (read `forge/loop.py`'s `write_tests` block first):**
...
- AFTER `_write_tests(frozen, visible)` (and `_write_tests(holdout, held)`), write the reference module into the frozen dir so the diff test can import it:
  ...
  ```
- Add the imports near the top of `loop.py`: `from forge.oracle import generate_oracle` (loop.py already imports `run_oracle_check` from `forge.oracle` — add `generate_oracle` to that import or a new line) and `from forge.examiner import TestFile` (confirm whether `TestFile` is already imported; add if not).
...
Run: `cd /Users/qatadaha/Coding/forge && source .venv/bin/activate && python -m pytest tests/test_loop.py::test_loop_oracle_converge_target -q`
...
```bash
cd /Users/qatadaha/Coding/forge && git add forge/loop.py tests/test_loop.py && git commit -m "feat: oracle converge target — fold the reference diff test into the frozen suite"
```
...
`forge solve <goal-dir>` with `forge.yaml` setting `oracle_converge_target: true` → the Builder must agree with an independent reference (fuzzed during the build), not just pass the Examiner's tests. A reference that contradicts the Examiner's suite makes the build fail (surfacing the conflict) rather than silently corrupting the solution.
