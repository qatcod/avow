# Forge — Adversarial-Escalating Examiner — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans. Steps use checkbox (`- [ ]`) syntax.

**Goal:** After a solution converges, have the Examiner read it and write harder tests aimed at its weak spots; the suite battle-hardens across escalation rounds via a `harden()` orchestrator + `forge harden` CLI.

**Architecture:** A new `Examiner.write_adversarial_tests(goal, solution_code)` method and `forge/harden.py` (wraps the existing `solve()`: converge → adversarial-escalate → re-converge), reusing `improve`'s suite-growth + last-known-good helpers. `solve()` is unchanged.

**Tech Stack:** Python 3.12 · `anthropic` structured outputs · reuses `forge.examiner`/`forge.improve`/`forge.loop`/`forge.config`.
...
**Files:**
- Modify: `/Users/qatadaha/Coding/forge/forge/examiner.py`
- Test: `/Users/qatadaha/Coding/forge/tests/test_examiner_adversarial.py`
...
Run: `cd /Users/qatadaha/Coding/forge && source .venv/bin/activate && python -m pytest tests/test_examiner_adversarial.py -q`
Expected: FAIL — `AttributeError: 'Examiner' object has no attribute 'write_adversarial_tests'`.

- [ ] **Step 3: Edit `forge/examiner.py`**
...
```bash
cd /Users/qatadaha/Coding/forge && git add forge/examiner.py tests/test_examiner_adversarial.py && git commit -m "feat: Examiner.write_adversarial_tests — harder tests targeting a passing solution"
```
...
```bash
cd /Users/qatadaha/Coding/forge && git add forge/config.py tests/test_config.py && git commit -m "feat: adversarial_rounds setting on RunConfig"
```
...
**Files:**
- Create: `/Users/qatadaha/Coding/forge/forge/harden.py`
- Test: `/Users/qatadaha/Coding/forge/tests/test_harden.py`
...
Run: `cd /Users/qatadaha/Coding/forge && source .venv/bin/activate && python -m pytest tests/test_harden.py -q`
Expected: FAIL — `ModuleNotFoundError: No module named 'forge.harden'`.

- [ ] **Step 3: Write `forge/harden.py`**
...
from forge.improve import _append_tests, _snapshot
...
```bash
cd /Users/qatadaha/Coding/forge && git add forge/harden.py tests/test_harden.py && git commit -m "feat: harden() — escalate the suite with adversarial tests, re-converge each round"
```

---

### Task 4: `forge harden` CLI

**Files:**
- Modify: `/Users/qatadaha/Coding/forge/forge/cli.py`
- Test: `/Users/qatadaha/Coding/forge/tests/test_cli_harden.py`

**Interfaces:**
- New subcommand: `forge harden <goal_dir> [--config forge.yaml] [--no-llm-verify]`. Builds the Examiner + Builder + a shared verify client (unless `--no-llm-verify`), runs `harden(...)` passing the client as `intent_client`/`property_client`/`oracle_client`, prints the verdict + per-round lines. The existing subcommands are unchanged.
...
Run: `cd /Users/qatadaha/Coding/forge && source .venv/bin/activate && python -m pytest tests/test_cli_harden.py -q`
...
def _cmd_harden(args) -> int:
    from forge.harden import harden
...
Run: `cd /Users/qatadaha/Coding/forge && source .venv/bin/activate && python -m pytest tests/test_cli_harden.py tests/test_cli.py -q`
...
Expected: all tests PASS, 0 warnings; `forge --help` lists `solve`, `improve`, `harden`, `mutate`, `intent-check`, `verify`, `propertize`, `oracle`.

- [ ] **Step 6: Prepare commit** (run only when greenlit)

```bash
cd /Users/qatadaha/Coding/forge && git add forge/cli.py tests/test_cli_harden.py && git commit -m "feat: forge harden CLI — build + adversarially battle-harden the suite"
```
...
`forge harden ~/Coding/forge-demo-full` → converges, then the Examiner reads the solution and writes harder tests targeting it; the suite grows and re-converges each round. Prints a line per round.
