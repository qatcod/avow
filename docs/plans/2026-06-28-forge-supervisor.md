# Forge — The Supervisor — Implementation Plan
...
**Architecture:** A new `forge/supervisor.py` (`review_trajectory`), an opt-in loop hook that fires once on plateau (redirect-hint or escalate-stop), and a `forge supervise` CLI. `solve()`'s floor (budget/plateau/stops) is unchanged and remains authoritative.

**Tech Stack:** Python 3.12 · `anthropic` structured outputs · integrates with `forge.loop`/`forge.config`/`forge.cli`.
...
**Files:**
- Create: `/Users/qatadaha/Coding/forge/forge/supervisor.py`
- Test: `/Users/qatadaha/Coding/forge/tests/test_supervisor.py`
...
from forge.supervisor import review_trajectory, SupervisorVerdict
...
Run: `cd /Users/qatadaha/Coding/forge && source .venv/bin/activate && python -m pytest tests/test_supervisor.py -q`
Expected: FAIL — `ModuleNotFoundError: No module named 'forge.supervisor'`.

- [ ] **Step 3: Write `forge/supervisor.py`**
...
```bash
cd /Users/qatadaha/Coding/forge && git add forge/supervisor.py tests/test_supervisor.py && git commit -m "feat: Supervisor review_trajectory — judge a plateauing run and recommend"
```
...
```bash
cd /Users/qatadaha/Coding/forge && git add forge/config.py tests/test_config.py && git commit -m "feat: dormant Supervisor settings on RunConfig"
```
...
**Interfaces:**
- `solve` gains keyword-only `supervisor_client=None` (after `oracle_client`). Imports `review_trajectory` from `forge.supervisor`.

**Edits (read `forge/loop.py` to confirm the exact current lines first):**
1. Import: `from forge.supervisor import review_trajectory`.
...
Run: `cd /Users/qatadaha/Coding/forge && source .venv/bin/activate && python -m pytest tests/test_loop.py::test_loop_supervisor_escalates -q`
Expected: FAIL — `TypeError: solve() got an unexpected keyword argument 'supervisor_client'`.

- [ ] **Step 3: Apply the edits above to `forge/loop.py`**
...
```bash
cd /Users/qatadaha/Coding/forge && git add forge/loop.py tests/test_loop.py && git commit -m "feat: event-triggered Supervisor hook in the loop (dormant by default; redirect/escalate)"
```

---

### Task 4: `forge supervise` CLI

**Files:**
- Modify: `/Users/qatadaha/Coding/forge/forge/cli.py`
- Test: `/Users/qatadaha/Coding/forge/tests/test_cli_supervise.py`

**Interfaces:**
- New subcommand: `forge supervise <run_jsonl> <goal_file> [--config]`. Reads the recorded `run.jsonl` into a trajectory, runs `review_trajectory`, prints the verdict. The existing subcommands are unchanged.
...
Run: `cd /Users/qatadaha/Coding/forge && source .venv/bin/activate && python -m pytest tests/test_cli_supervise.py -q`
...
Run: `cd /Users/qatadaha/Coding/forge && source .venv/bin/activate && python -m pytest tests/test_cli_supervise.py tests/test_cli.py -q`
...
Expected: all tests PASS, 0 warnings; `forge --help` lists `solve`, `improve`, `harden`, `population`, `mutate`, `intent-check`, `verify`, `propertize`, `oracle`, `supervise`.

- [ ] **Step 6: Prepare commit** (run only when greenlit)

```bash
cd /Users/qatadaha/Coding/forge && git add forge/cli.py tests/test_cli_supervise.py && git commit -m "feat: forge supervise CLI — review a recorded run's trajectory"
```
...
`forge supervise ~/Coding/forge-demo-full/.forge/run.jsonl ~/Coding/forge-demo-full/goal.md` → prints the Supervisor's read of that run. To see it in-loop, run `forge solve` on a hard goal with a `forge.yaml` setting `supervisor_enabled: true` (then it fires once on plateau and may redirect or escalate).
