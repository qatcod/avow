# Avow v2.2 — Back-Translation Intent Check — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans. Steps use checkbox (`- [ ]`) syntax.

**Goal:** A back-translation intent check — a model reads the test suite *blind*, reconstructs the goal, and a judge scores how well that matches the real goal — exposed as a standalone `avow intent-check` CLI and an early report hook in the loop (catches "Examiner built to the wrong goal" before the build budget is spent).

**Architecture:** A new `avow/backtranslation.py` with two structured-output LLM calls (blind back-translate, then judge), behind an injectable client. Cross-model by default (back-translator ≠ Examiner). Surfaced via a CLI and an early `solve()` hook.

**Tech Stack:** Python 3.12 · `anthropic` structured outputs · `pydantic` · integrates with `avow.config`/`avow.loop`/`avow.budget`/`avow.memory`.
...
**Files:**
- Create: `/Users/qatadaha/Coding/avow/avow/backtranslation.py`
- Test: `/Users/qatadaha/Coding/avow/tests/test_backtranslation_translate.py`
...
Run: `cd /Users/qatadaha/Coding/avow && source .venv/bin/activate && python -m pytest tests/test_backtranslation_translate.py -q`
Expected: FAIL — `ModuleNotFoundError: No module named 'avow.backtranslation'`.

- [ ] **Step 3: Write `avow/backtranslation.py`**
...
```bash
cd /Users/qatadaha/Coding/avow && git add avow/backtranslation.py tests/test_backtranslation_translate.py && git commit -m "feat: blind back-translation of a test suite into an inferred goal"
```
...
**Files:**
- Modify: `/Users/qatadaha/Coding/avow/avow/backtranslation.py`
- Test: `/Users/qatadaha/Coding/avow/tests/test_backtranslation_judge.py`
...
Run: `cd /Users/qatadaha/Coding/avow && source .venv/bin/activate && python -m pytest tests/test_backtranslation_judge.py -q`
Expected: FAIL — `ImportError: cannot import name 'judge_intent_match'`.

- [ ] **Step 3: Append to `avow/backtranslation.py`**
...
```bash
cd /Users/qatadaha/Coding/avow && git add avow/backtranslation.py tests/test_backtranslation_judge.py && git commit -m "feat: intent-match judge comparing original vs inferred goal"
```
...
**Files:**
- Modify: `/Users/qatadaha/Coding/avow/avow/backtranslation.py`
- Test: `/Users/qatadaha/Coding/avow/tests/test_backtranslation_run.py`
...
Run: `cd /Users/qatadaha/Coding/avow && source .venv/bin/activate && python -m pytest tests/test_backtranslation_run.py -q`
...
```bash
cd /Users/qatadaha/Coding/avow && git add avow/backtranslation.py tests/test_backtranslation_run.py && git commit -m "feat: run_intent_check orchestrates blind back-translate then judge"
```
...
```bash
cd /Users/qatadaha/Coding/avow && git add avow/config.py tests/test_config.py && git commit -m "feat: back-translation settings on RunConfig"
```

---

### Task 5: `avow intent-check` CLI (the spin-out)

**Files:**
- Modify: `/Users/qatadaha/Coding/avow/avow/cli.py`
- Test: `/Users/qatadaha/Coding/avow/tests/test_cli_intent.py`

**Interfaces:**
- New subcommand: `avow intent-check <goal_file> <tests_dir> [--config avow.yaml]`. Builds `anthropic.Anthropic()`, runs `run_intent_check`, prints the match score, the inferred goal, and the divergences. The existing `solve` and `mutate` subcommands are unchanged.
...
Run: `cd /Users/qatadaha/Coding/avow && source .venv/bin/activate && python -m pytest tests/test_cli_intent.py -q`
...
Run: `cd /Users/qatadaha/Coding/avow && source .venv/bin/activate && python -m pytest tests/test_cli_intent.py tests/test_cli.py tests/test_cli_mutate.py -q`
Expected: PASS (intent-check + the unchanged solve and mutate tests).

- [ ] **Step 5: Prepare commit** (run only when greenlit)

```bash
cd /Users/qatadaha/Coding/avow && git add avow/cli.py tests/test_cli_intent.py && git commit -m "feat: avow intent-check CLI — back-translation suite/goal match scoring"
```
...
- `solve` gains a keyword-only `intent_client=None` (after `mutation_client`). Imports `run_intent_check` from `avow.backtranslation`.
- After the Examiner writes the suite AND the confirm gate passes (inside the `if write_tests:` block), IF `config.intent_check_enabled and intent_client is not None`: run `run_intent_check(goal, frozen, intent_client, config.backtranslation_model)`, charge `budget.charge_tokens(config.backtranslation_model, ir.input_tokens, ir.output_tokens)`, set `intent_score = ir.score`, and record an `AttemptRecord` summarizing it (via a fresh `RunLog(avow_dir / "run.jsonl")`). The score is surfaced on the `"green"`, `"overfit_on_holdout"`, and final break returns (it is computed before the loop). The `"aborted"` return keeps `intent_score=None` (the check runs after the gate).
...
Run: `cd /Users/qatadaha/Coding/avow && source .venv/bin/activate && python -m pytest tests/test_loop.py::test_loop_records_intent_score -q`
...
```bash
cd /Users/qatadaha/Coding/avow && git add avow/loop.py tests/test_loop.py && git commit -m "feat: early intent-check hook records intent_score on SolveResult"
```
...
`avow intent-check ~/Coding/avow-demo/goal.md ~/Coding/avow-demo/tests_frozen` → prints the goal the tests *appear* to verify, the match score, and any divergences. Point it at a suite you suspect is testing the wrong thing to see it flag the drift.
