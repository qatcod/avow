# Forge — Population / Hybrid Search — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans. Steps use checkbox (`- [ ]`) syntax.

**Goal:** Run N candidate solutions against the same suite and let the verifier pick the winner (`population_solve`), plus an escalate-on-plateau wrapper (`hybrid_solve`) and a `forge population` CLI.

**Architecture:** A new `forge/population.py` reusing the unchanged `solve()` per candidate with isolated workspaces that share a copy of the suite; `select_best` ranks by confidence. `solve()` is not refactored.

**Tech Stack:** Python 3.12 · reuses `forge.loop.solve`/`SolveResult` · `forge.improve._snapshot` · integrates with `forge.config`/`forge.cli`.
...
**Files:**
- Create: `/Users/qatadaha/Coding/forge/forge/population.py`
- Test: `/Users/qatadaha/Coding/forge/tests/test_population_select.py`
...
Run: `cd /Users/qatadaha/Coding/forge && source .venv/bin/activate && python -m pytest tests/test_population_select.py -q`
Expected: FAIL — `ModuleNotFoundError: No module named 'forge.population'`.

- [ ] **Step 3: Write `forge/population.py`**
...
```bash
cd /Users/qatadaha/Coding/forge && git add forge/population.py tests/test_population_select.py && git commit -m "feat: select_best — rank candidate solutions by success, confidence, score"
```
...
```bash
cd /Users/qatadaha/Coding/forge && git add forge/config.py tests/test_config.py && git commit -m "feat: population_size setting on RunConfig"
```
...
**Files:**
- Modify: `/Users/qatadaha/Coding/forge/forge/population.py`
- Test: `/Users/qatadaha/Coding/forge/tests/test_population.py`
...
Run: `cd /Users/qatadaha/Coding/forge && source .venv/bin/activate && python -m pytest tests/test_population.py -q`
Expected: FAIL — `ImportError: cannot import name 'population_solve'`.

- [ ] **Step 3: Append to `forge/population.py`**
...
```bash
cd /Users/qatadaha/Coding/forge && git add forge/population.py tests/test_population.py && git commit -m "feat: population_solve + hybrid_solve — N candidates, verifier picks the winner"
```

---

### Task 4: `forge population` CLI

**Files:**
- Modify: `/Users/qatadaha/Coding/forge/forge/cli.py`
- Test: `/Users/qatadaha/Coding/forge/tests/test_cli_population.py`

**Interfaces:**
- New subcommand: `forge population <goal_dir> [--config forge.yaml] [--no-llm-verify] [--hybrid]`. Builds the Examiner + Builder + a shared verify client (unless `--no-llm-verify`), runs `hybrid_solve` (with `--hybrid`) or `population_solve`, prints the winner + a line per candidate. The existing subcommands are unchanged.
...
def test_forge_population_cli(tmp_path, capsys, monkeypatch):
    (tmp_path / "goal.md").write_text("Build add(a, b) returning a + b.")
    import anthropic
...
Run: `cd /Users/qatadaha/Coding/forge && source .venv/bin/activate && python -m pytest tests/test_cli_population.py -q`
...
Run: `cd /Users/qatadaha/Coding/forge && source .venv/bin/activate && python -m pytest tests/test_cli_population.py tests/test_cli.py -q`
...
Expected: all tests PASS, 0 warnings; `forge --help` lists `solve`, `improve`, `harden`, `population`, `mutate`, `intent-check`, `verify`, `propertize`, `oracle`.

- [ ] **Step 6: Prepare commit** (run only when greenlit)

```bash
cd /Users/qatadaha/Coding/forge && git add forge/cli.py tests/test_cli_population.py && git commit -m "feat: forge population CLI — N-candidate search, verifier picks the winner (--hybrid)"
```
...
`forge population ~/Coding/forge-demo-full --config <caps>` → builds N candidate solutions against the same suite and reports which the verifier picked (and each candidate's confidence). `--hybrid` runs one attempt first and only escalates if it doesn't go green-and-confident.
